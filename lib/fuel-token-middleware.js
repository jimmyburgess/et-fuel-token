	...
	var getNewTokenFromSession = function( req, next, attempt ) {
		...
		req.sessionStore.get( req.sessionID, function( error, savedSession ) {
			if (
				...
				return next( new Error( 'fuel: refresh reload hit max attempts, token could not be refreshed' ) );
			}
			
			setTimeout( getNewTokenFromSession.bind( null, req, next, attempt+1 ), options.tokenReloadWait );
		} );
	};
